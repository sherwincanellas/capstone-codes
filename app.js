    ...
    if(getData.length > 0){
        for(var i=tab_start; i<tab_end; i++){
            ...
            if(student){
                ...
                <td>${i+1}</td>
                <td><img src="${student.picture}" alt="" width="40" height="40"></td>
                <td>${student.fName + " " + student.lName}</td>
                <td>${student.studentNumVal}</td>
                <td>${student.addressVal}</td>
                <td>${student.courseVal}</td>
                <td>${student.yearlevelVal}</td>
                <td>${student.bDateVal}</td>
                <td>${student.emailVal}</td>
                <td>${student.phoneVal}</td>
                <td>
                    <button onclick="readInfo('${student.picture}', '${student.fName}', '${student.lName}', '${student.studentNumVal}', '${student.addressVal}', '${student.courseVal}', '${student.yearlevelVal}', '${student.bDateVal}', '${student.emailVal}', '${student.phoneVal}')"><i class="fa-regular fa-eye"></i></button>
                    <button onclick="editInfo('${i}', '${student.picture}', '${student.fName}', '${student.lName}', '${student.studentNumVal}', '${student.addressVal}', '${student.courseVal}', '${student.yearlevelVal}', '${student.bDateVal}', '${student.emailVal}', '${student.phoneVal}')"><i class="fa-regular fa-pen-to-square"></i></button>
...
function readInfo(pic, fname, lname, studentNum, address, course, yearlevel, bDate, Email, Phone){
    imgInput.src = pic
    fName.value = fname
    lName.value = lname
...
function editInfo(id, pic, fname, lname, studentNum, address, course, yearlevel, bDate, Email, Phone){
    isEdit = true
    editId = id
    ...
    originalData[originalIndex] = {
        ...
        fName: fname,
        lName: lname,
        studentNumVal: studentNum,
        addressVal: address,
        courseVal: course,
        yearlevelVal: yearlevel,
        bDateVal: bDate,
        emailVal: Email,
        phoneVal: Phone
    }
...
form.addEventListener('submit', (e)=> {
    ...
    const information = {
        id: Date.now(),
        picture: imgInput.src == undefined ? "user.png" :imgInput.src,
        fName: fName.value,
        lName: lName.value,
        studentNumVal: studentNum.value,
        addressVal: address.value,
        courseVal: course.value,
        yearlevelVal: yearlevel.value,
...
filterData.addEventListener("input", ()=> {
    ...
    if(searchTerm !== ""){
        ...
        const filteredData = originalData.filter((item) => {
            ...
            const address = item.addressVal.toLowerCase()
